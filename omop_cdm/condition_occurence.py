...
class ConditionOccurrence:
    ...
    PostgreSQL 'public.condition_occurrence' table.
    """

    @classmethod
    def ingest(cls):
        """
        Reads the condition_occurrence Parquet file, transforms the data,
        and loads it into the PostgreSQL 'public.condition_occurrence' table.
        """
        parquet_file_path = "parquet/tb_condition_occurence_data.parquet"  # Can be HDFS, S3, local, etc.
        POSTGRES_TABLE = "condition_occurrence"
        ...
        try:
            ...
            print("Parquet file schema for condition_occurrence:")
            df_parquet.printSchema()
            print("First 5 rows of Parquet data for condition_occurrence:")
            df_parquet.show(5)

            # Select and cast columns to match the PostgreSQL 'condition_occurrence' table schema.
            ...
            df_transformed = df_parquet.select(
                col("condition_occurrence_id").cast(IntegerType()),
                col("person_id").cast(IntegerType()),
                col("condition_concept_id").cast(IntegerType()),
                col("condition_start_date").cast(DateType()),
                # condition_start_datetime not in parquet, will be NULL in PG
                # condition_end_date not in parquet, will be NULL in PG
                # condition_end_datetime not in parquet, will be NULL in PG
                col("condition_type_concept_id").cast(IntegerType()),
                # condition_status_concept_id not in parquet, will be NULL in PG
                ...
                # condition_source_value not in parquet, will be NULL in PG
                # condition_source_concept_id not in parquet, will be NULL in PG
                # condition_status_source_value not in parquet, will be NULL in PG
            )
            ...
            print(f"An error occurred during ConditionOccurrence ingestion: {e}")
